...
public final class InputManager {
    ...
    // 缓存的 Method 对象 - 使用 volatile 确保线程安全
    private static volatile Method injectInputEventMethod;
    private static volatile Method setDisplayIdMethod;
    private static volatile Method setActionButtonMethod;
    private static volatile Method addUniqueIdAssociationByPortMethod;
    private static volatile Method removeUniqueIdAssociationByPortMethod;

    // 静态初始化 - 预加载常用方法减少运行时反射开销
    static {
        try {
            ...
            Ln.w("Could not cache injectInputEvent method");
        }
        try {
            setDisplayIdMethod = InputEvent.class.getMethod("setDisplayId", int.class);
        } catch (NoSuchMethodException e) {
            Ln.w("Could not cache setDisplayId method");
        }
        try {
            setActionButtonMethod = MotionEvent.class.getMethod("setActionButton", int.class);
        } catch (NoSuchMethodException e) {
            Ln.w("Could not cache setActionButton method");
        }
    }
