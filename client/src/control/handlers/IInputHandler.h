...
/**
 * @brief 输入处理器接口 / Input Handler Interface (纯 C++，无 QObject)
 *
...
public:
     ...
     * @return true 表示事件已被消费，不再传递；false 继续传递
     */
    virtual bool handleMouseEvent(const InputEvent& event,
                                  const Size& frameSize,
                                  const Size& showSize) {
        (void)event;
        (void)frameSize;
     ...
     * @return true 表示事件已被消费，不再传递；false 继续传递
     */
    virtual bool handleWheelEvent(const InputEvent& event,
                                  const Size& frameSize,
                                  const Size& showSize) {
        (void)event;
        (void)frameSize;
