...
void QYUVOpenGLWidget::updateTextureWithPBONoContext(GLuint texture, quint32 textureType, quint8 *pixels, quint32 stride)
{
    ...
    if (stride != static_cast<quint32>(size.width())) {
        ...
        for (int y = 0; y < size.height(); ++y) {
            memcpy(tempBuffer.data() + y * size.width(), pixels + y * stride, size.width());
        }
        glBufferSubData(GL_PIXEL_UNPACK_BUFFER, 0, dataSize, tempBuffer.data());
    } else {
...
void QYUVOpenGLWidget::updateTextureWithPBO(GLuint texture, quint32 textureType, quint8 *pixels, quint32 stride)
{
    ...
    // 如果数据有 padding，需要逐行拷贝
    if (stride != static_cast<quint32>(size.width())) {
        // 有 padding，需要逐行拷贝到临时缓冲区
        std::vector<uint8_t> tempBuffer(dataSize);
        for (int y = 0; y < size.height(); ++y) {
            memcpy(tempBuffer.data() + y * size.width(), pixels + y * stride, size.width());
        }
...
void QYUVOpenGLWidget::paintGL()
{
    ...
    m_totalRenderTime += m_renderTimer.nsecsElapsed() / 1000000.0;
...
void QYUVOpenGLWidget::updateTexture(GLuint texture, quint32 textureType, quint8 *pixels, quint32 stride)
{
    ...
    qDebug() << "[TEX-UPDATE]" << QDateTime::currentDateTime().toString("hh:mm:ss.zzz")
             << "type=" << textureType << "calling makeCurrent...";
    makeCurrent();
    qDebug() << "[TEX-UPDATE]" << QDateTime::currentDateTime().toString("hh:mm:ss.zzz")
             << "type=" << textureType << "makeCurrent done, uploading...";
    ...
    qDebug() << "[TEX-UPDATE]" << QDateTime::currentDateTime().toString("hh:mm:ss.zzz")
             << "type=" << textureType << "upload done, calling doneCurrent...";
    doneCurrent();
    qDebug() << "[TEX-UPDATE]" << QDateTime::currentDateTime().toString("hh:mm:ss.zzz")
             << "type=" << textureType << "doneCurrent done";
}
